...
function AdminSignup() {
  ...
  const submit = () => {
    ...
    else 
    {
      ...
      axios.post('http://localhost:4000/api/adminsign', { name, email, password }).then((res) =>
       {
  ...
  return (
    ...
    <MDBCard className='m-5' style={{maxWidth: '600px'}}>
      <MDBCardBody className='px-5'>
        <h2 className="text-uppercase text-center mb-5">Admin</h2>
        <MDBInput wrapperClass='mb-4' label='Admin Name' size='lg' id='form1' type='text'  onChange={(e) => {
                    setname(e.target.value)
                  }}/>
        <MDBInput wrapperClass='mb-4' label='Admin Email' size='lg' id='form2' type='email'  onChange={(e) => {
